...
def get_vlocG(cell, Gv=None):
    ...
    for x in range(0, len(Gv)):
       g2.append(np.dot(Gv[x],Gv[x]))
    g2=np.array(g2)   

    print 'test'  
    #Gvnorm = lib.norm(Gv, axis=1)
    vlocG = get_gth_vlocG(cell, g2)
    vlocG[:,0] = 0.
    return vlocG
    ...
    for ia in range(cell.natm):
        ...
        cfacs = np.array(
                [1*G_red**0,
                 3 - G_red,
                 15 - 10*G_red + G_red**2,
                 105 - 105*G_red + 21*G_red**2 - G_red**3])

        # Note the signs -- potential here is positive
        vlocG[ia,:] = ( Zia * coulG * np.exp(-0.5*G_red)
                       - (2*np.pi)**(3/2.)*rloc**3*np.exp(-0.5*G_red)*(
...
def get_pp(cell, Gd, rd, kpt=np.zeros(3)):
