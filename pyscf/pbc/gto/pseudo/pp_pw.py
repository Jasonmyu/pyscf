...
def get_projG_pwgrid(cell, Gv, kpt=np.zeros(3)):
    return get_gth_projG(cell, kpt+Gv)
...
def get_pp(cell, Gd, rd, v, GH, gs, GH_ind, kpt=np.zeros(3)):
    ...
    nao = len(GH)
    SI = cell.get_SI_pw(Gd)
    vlocG = get_vlocG(cell,Gd)
    vpplocG = -np.sum(SI * vlocG, axis=0)
    vpplocG[0] = np.sum(get_alphas(cell)) # from get_jvloc_G0 function

    # vpploc evaluated in real-space
    # add "cell." in front of "gs" to revert
    vpplocR = tools.ifft(vpplocG, gs).real
